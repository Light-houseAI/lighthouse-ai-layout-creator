...
const AnalyzeAnimation = ({ isActive }: AnalyzeAnimationProps) => {
  ...
  return (
    <div className="absolute inset-0 flex items-center justify-center gap-12">
      ...
      <div className="relative w-48 h-80">
        {inputSources.map((source, index) => {
          ...
          return (
            <div
              key={source.label}
              className={`absolute bg-card rounded-lg shadow-md border border-border px-3 py-2 flex items-center gap-2 transition-all duration-500 ${
                isVisible 
      ...
      <div className="flex flex-col items-center">
        <div 
          className={`relative w-16 h-16 rounded-full flex items-center justify-center transition-all duration-500 ${
            phase === 'processing' ? 'bg-accent scale-110' : phase === 'output' ? 'bg-accent' : 'bg-muted'
          }`}
        >
          {/* Pulsing ring */}
          <div 
            className={`absolute inset-0 rounded-full border-2 border-accent transition-opacity ${
        ...
        <p className={`text-[10px] text-muted-foreground mt-3 tracking-wide transition-opacity ${
      ...
      <div 
        className={`w-64 bg-accent rounded-xl overflow-hidden shadow-lg transition-all duration-700 ${
